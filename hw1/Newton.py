...
sspTemplate = np.dot(Poincare.zRotation(thetaPoincare), e_x)
# Normal to this plane will be equal to template vector rotated pi/2 about
# the z axis
nTemplate = np.dot(Poincare.zRotation(pi / 2), sspTemplate)
...
Aeq0 = Rossler.StabilityMatrix(eq0)
...
for i in range(np.size(sspSolution, 0) - 1):
    ...
    if UPoincare(sspSolution[i]) < 0 < UPoincare(sspSolution[i+1]):
        ...
        # Define the equation for deltat which must be solved as a lambda func
...
PoincareSection = PoincareSection.transpose()
...
ReturnMap = lambda s: interpolate.splev(s, tckReturn) - s
...
while np.max(np.abs(error)) > tol:
    ...
    Newton[0:3, 0:3] = 1 - Rossler.Jacobian(sspfixed, period)
