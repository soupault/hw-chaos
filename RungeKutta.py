...
def RK4(velocityFunction, initialCondition, timeArray):
    ...
    for i in range(0, np.size(timeArray) - 1):
        ...
        k2 = deltat * velocityFunction(SolutionArray[i] + k1/2, timeArray[i])
        k3 = deltat * velocityFunction(SolutionArray[i] + k2/2, timeArray[i])
        k4 = deltat * velocityFunction(SolutionArray[i] + k3, timeArray[i])
        # Next integration step
        SolutionArray[i + 1] = SolutionArray[i] + k1/6 + k2/3 + k3/3 + k4/6
